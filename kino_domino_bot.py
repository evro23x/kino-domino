...
from db_quering import get_current_movie_id, main_search, get_theater_by_name, prepare_theater_timetable
...
GET_A_MOVIE_NAME, ANALYZE_USER_LOCATION, WHAT_TO_DO_NEXT, GET_A_SIMILAR_MOVIE, GET_A_CINEMA_CHOOSE, GET_A_CINEMA_NAME, \
    GET_A_CINEMA_CHOOSE_BY_DATE = range(7)
...
def get_a_cinema_name(bot, update):
    ...
    USER_INPUT['theater'] = get_theater_by_name(update.message.text)
    add_log(update, msg_in='Название кинотеатра = ' + str(update.message.text), msg_out='')
    not_found_error_msg = "Извини, но я не смог найти похожего на {} названия кинотетра. Попробуй еще раз.".format(
        update.message.text)
    if USER_INPUT['theater'] is None:
...
def get_a_cinema_choose_by_date(bot, update):
    ...
    if [update.message.text] == variants[0]:
        ...
        timetable = prepare_theater_timetable(theater_id, date.today() + timedelta(2), date.today() + timedelta(3))
